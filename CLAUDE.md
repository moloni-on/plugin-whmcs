...
- **Models** (`src/MoloniOn/Models/`): `Order`, `Document`, `Config`, `Log`, `Auth` (extend `AbstractModel`); `Whmcs` reads native WHMCS tables (including `adminLanguage()` — the current admin's language from `tbladmins` via `$_SESSION['adminid']`, falling back to the system default language in `tblconfiguration` when the admin is on "Default"; this drives UI locale detection, see below)
- **Support** (`src/MoloniOn/Support/`): `Platform` (endpoints), `Context` (per-request session/token/company), `Company` (company payload + feature permissions via `limits`), `Lang` (i18n; `boot()` resolves the admin locale to `pt`/`en`, tolerant of ISO codes and varied WHMCS language slugs — `pt`, `portuguese-pt`, `portugues`, …; see UI-locale detection below), `Template` (renderer), `Request` (typed view over the request superglobals, keeps `Dispatcher` testable), `FiscalZone` (code + countryId value object; centralises the `PT`/upper-case defaults), `LineInput` (billing fields of one document line: name/price/reference/summary/discount), `CurrencyExchange` (resolved exchange id + rate value object; `toBase()` converts an order-currency amount to the company base currency), `Paginator` (immutable page state — items + totals — for the list views; `PER_PAGE`=15, built via `fromSlice()` for in-memory lists or `paginate()` for DB queries), `Hooks` (custom extension points fired via WHMCS `run_hook()`; `filter()`/`doAction()`/`allows()`, no-op when `run_hook` is absent — see below)
...
- **i18n**: no hardcoded UI strings — use `lang/en.php` and `lang/pt.php` (the module's own translation tables, keyed arrays consumed by `Support\Lang`). Portuguese addresses the user in the second person, informal ("tu"); system action results use neutral/passive phrasing (e.g. "Configurações guardadas.").
  - **UI-locale detection**: which of `en`/`pt` renders is decided by `Lang::boot(Whmcs::adminLanguage())` in `Dispatcher::dispatch()`. `Whmcs::adminLanguage()` returns the admin's language name from `tbladmins` (falling back to the system default in `tblconfiguration`); `Lang::boot()` maps it to `pt`/`en` via `isPortuguese()`, which matches the `portug` stem or a `pt` prefix. This deliberately tolerates non-standard WHMCS language slugs seen in the wild — e.g. `portugues` (no trailing "e"), `português`, `portuguese-pt` — since the exact name varies per install. (WHMCS's own `$vars['_lang']`/`$_ADDONLANG` marker mechanism was tried and rejected: it depends on shipping a file named for each possible admin-language slug, which breaks on custom slugs like `portugues`. See journals 043–045.) Adding a new UI language means adding a `lang/<code>.php` table and extending `isPortuguese()`-style detection.
